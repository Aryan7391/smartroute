...
def inject_order(order_id: str, background_tasks: BackgroundTasks = None) -> dict:
    ...
    for vehicle in vehicles:
        ...
        if vehicle["status"] == "idle":
            if all_idle or dist_to_pickup <= PROXIMITY_KM:
                if order["approx_weight"] <= vehicle.get("max_weight", float("inf")):
                    return _assign_to_idle(vehicle, order, background_tasks)
            continue
        ...
        if vehicle["status"] == "active":
            ...
            return _insert_into_route(vehicle, order, remaining, best_insert, background_tasks)

    return _queue_order(order_id, "no vehicle matched")


def _assign_to_idle(vehicle: dict, order: dict, background_tasks: BackgroundTasks = None) -> dict:
...
def _insert_into_route(vehicle: dict, order: dict, remaining: list, insert_at: int, background_tasks: BackgroundTasks = None) -> dict:
    # Compute new sequences BEFORE any DB writes to avoid a race between
    # shifting downstream stops and deriving pickup_seq from the pre-shift values.
    if insert_at == 0:
        pickup_seq = 1
    else:
        pickup_seq = remaining[insert_at - 1]["sequence"] + 1
    delivery_seq = pickup_seq + 1

    # Shift all downstream stops up by 2 to make room
